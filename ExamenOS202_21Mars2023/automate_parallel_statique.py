...
begin = rank*(nombre_cas//nbp)
end = (rank+1)*(nombre_cas//nbp)
...
for num_config in range(begin,end): # save une image for each case -> On peut paralléliser cela.
    t1 = time.time()
    ...
    for iter in range(1, nb_iterations):
        vals = np.left_shift(1, 4*cells[iter-1, 0:-2] #fait les iterations par ligne 
                             + 2*cells[iter-1, 1:-1]
                             + cells[iter-1, 2:])
    ...
    # gather_data = globCom.gather(cells[begin:end,:],root=0)
