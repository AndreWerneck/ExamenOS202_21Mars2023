...
#division statique des données

begin = rank*taille_nuage/nbp
end = (rank+1)*taille_nuage/nbp
...
for r in range(nbre_repet):
    t1 = time.time()
    # on parallelise le nuage -> calcul locale
    nuage = np.array(np.array([[resolution_x * i * math.cos(48371.*i)/taille_nuage for i in range(begin,end)], [resolution_y * math.sin(50033./(i+1.)) for i in range(begin,end)]], dtype=np.float64).T)
    t2 = time.time()
...
#gather
# gather_data = globCom.gather(nuage)
#echange des données
for i in range(nbp):
    if(rank%2):
        globCom.send(nuage,i+1)
        data_recv = globCom.recv(source=i-1)
    else:
        globCom.send(nuage,i-1)
        data_recv= globCom.recv(source=i+1)
